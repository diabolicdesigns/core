...
if ( !class_exists( "calevents_raids_rss_crontask" ) ) {
    class calevents_raids_rss_crontask extends crontask {
        ...
        public function run(){
            ...
            // Function to format time in 12-hour format with AM/PM and timezone abbreviation
            function formatTime12HourWithTimezoneAbbr($timestamp, $timezone) {
                $date = new DateTime("@$timestamp");
                $date->setTimezone(new DateTimeZone($timezone));
                return $date->format('m-d-Y h:i A T'); // 'T' gives the timezone abbreviation
            }
            ...
            if (is_array($arrRaidIDlist)){
                foreach ($arrRaidIDlist as $intRaidID){
                    ...
                    $calendarName   = $this->pdh->get('calendars', 'name', array($this->pdh->get('calendar_events', 'calendar_id', array($intRaidID))));
                    ...
                    $rssitem->title         = $calendarName.' - '.formatTime12HourWithTimezoneAbbr($this->pdh->get('calendar_events', 'time_start', array($intRaidID)), $timezone).': '.$this->pdh->get('calendar_events', 'name', array($intRaidID));
            ...
            //====================
            // Just the Events
            $rssfile            = $this->pfh->FilePath('rss/calendar_events.xml', 'eqdkp', 'relative');
            $feed               = registry::register('feed');
            $feed->feedfile     = $this->pfh->FileLink('rss/calendar_events.xml', 'eqdkp', 'absolute');
            $feed->link         = $this->env->link;
            $feed->title        = "Calendar Events";
            $feed->description  = $this->config->get('main_title')." EQdkp-Plus - Calendar Events";
            $feed->published    = $this->time->time;
            $feed->language     = 'EN-EN';
            ...
            $arrRaidIDlist = $this->pdh->get('calendar_events', 'id_list', array('appointments', $this->time->time));
            $arrRaidIDlist = $this->pdh->sort($arrRaidIDlist, 'calendar_events', 'date', 'asc');
            if (is_array($arrRaidIDlist)) {
                $arrRaidIDlist = array_slice($arrRaidIDlist, 0, $intNumber);
            }
            ...
            if (is_array($arrRaidIDlist)){
                foreach ($arrRaidIDlist as $intRaidID){
                    ...
                    $rssitem = registry::register('feeditems', array(), $intRaidID);
                    $rssitem->title         = $calendarName.' - '.formatTime12HourWithTimezoneAbbr($this->pdh->get('calendar_events', 'time_start', array($intRaidID)), $timezone).': '.$this->pdh->get('calendar_events', 'name', array($intRaidID));
                    $rssitem->description   = $eventdata['notes'].(strlen($eventdata['extension']['location']) ? ' - '.$eventdata['extension']['location'] : '');
                    $rssitem->link          = $this->env->link.$this->routing->build('calendarevent', $this->pdh->get('calendar_events', 'name', array($intRaidID)), $intRaidID, false, true);
                    $rssitem->published     = $this->pdh->get('calendar_events', 'time_start', array($intRaidID));
                    $rssitem->author        = ($eventextension['raidleader'] > 0) ? implode(', ', $this->pdh->aget('member', 'name', 0, array($eventextension['raidleader']))) : '';
                    $rssitem->source        = $feed->link;
            ...
            $rssfile            = $this->pfh->FilePath('rss/calendar_all.xml', 'eqdkp', 'relative');
            $feed               = registry::register('feed');
            $feed->feedfile     = $this->pfh->FileLink('rss/calendar_all.xml', 'eqdkp', 'absolute');
            $feed->link         = $this->env->link;
            $feed->title        = "Calendar";
            $feed->description  = $this->config->get('main_title')." EQdkp-Plus - Calendar";
            $feed->published    = $this->time->time;
            $feed->language     = 'EN-EN';
